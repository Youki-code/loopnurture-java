...
CREATE TABLE marketing_user (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- 用户ID，自增主键
    ...
    extends_info JSONB,                         -- 用户扩展信息，存储JSON格式的扩展字段（包含OAuth信息等）
    ...
    -- 当前组织编码（多组织场景下标识当前激活组织）
    current_org_code VARCHAR(50),
...
CREATE TABLE organization (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- 组织ID，自增主键
    ...
    extends_info JSONB,                       -- 组织设置，使用JSONB存储OrganizationSettingsVO
    deleted BOOLEAN NOT NULL DEFAULT false,    -- 软删除标记
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_by BIGINT NOT NULL,                -- 创建人ID，关联marketing_user.id
    updated_by BIGINT NOT NULL                 -- 更新人ID，关联marketing_user.id
);
...
CREATE TABLE user_organization (
    ...
    CONSTRAINT fk_user_org_org FOREIGN KEY (org_code) REFERENCES organization(org_code),
    CONSTRAINT uk_user_org UNIQUE (system_user_id, org_code)
);

-- 创建用户-组织关联表索引
CREATE INDEX idx_user_org_user_id ON user_organization(system_user_id);
CREATE INDEX idx_user_org_org_code ON user_organization(org_code);
...
-- Add deleted to user_organization table
ALTER TABLE user_organization ADD COLUMN IF NOT EXISTS deleted BOOLEAN NOT NULL DEFAULT false; 