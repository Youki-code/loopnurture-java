...
CREATE TABLE marketing_email_template (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- 模板ID，自增主键
...
CREATE TABLE email_send_record (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- 记录ID，自增主键
    org_code VARCHAR(36) NOT NULL,              -- 组织编码
    rule_id BIGINT,                            -- 规则ID，关联email_send_rule.id
    template_id VARCHAR(50) NOT NULL,           -- 模板编码
    ...
    send_status SMALLINT NOT NULL,              -- 发送状态：1=PENDING, 2=SENDING, 3=SENT, 4=FAILED
...
CREATE INDEX idx_email_record_status ON email_send_record(send_status);
...
CREATE TABLE email_send_rule (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- 规则ID，自增主键
    org_code VARCHAR(36) NOT NULL,              -- 组织编码
